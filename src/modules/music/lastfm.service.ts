...
export class LastfmService {
	...
	async formatResult(
		...
		try {
			...
			fetchResult.map((data, index) => {
				const coverRegex =
					/<meta[^>]*(?:property|name)=["'](og:)(image)["'][^>]*content=["']([^"']+)["'][^>]*>/gi.exec(
						data.data,
					);
				if (filter == SEARCH_FILTERS.track) {
					const albumNameRegex =
						/<a[^>]*class="link-block-target"[^>]*>([^<]+)<\/a>/gi.exec(
							data.data,
						);
					const durationRegex =
						/<dd[^>]*class="catalogue-metadata-description"[^>]*>([^<]+)<\/dd>/gi.exec(
							data.data,
						);
					if (albumNameRegex && albumNameRegex[1])
						result[index]["albumName"] = albumNameRegex[1];
					if (durationRegex && durationRegex[1]) {
						const duration = durationRegex[1].trim().split(":");
						let finalDuration =
							parseInt(
								duration[duration.length - 2].substring(
									duration[duration.length - 2].length - 2,
								),
							) *
								60 +
							parseInt(duration[duration.length - 1]);
					...
					const listenersRegex = /<abbr[^>]*\stitle="([^"]*)"[^>]*>/gi.exec(
						data.data,
					);
					if (listenersRegex && listenersRegex[1])
						result[index]["listeners"] = parseInt(
							listenersRegex[1].replaceAll(",", ""),
						);
				}
				if (coverRegex && coverRegex[3]) result[index][key] = coverRegex[3];
				finalResult.push(
					applyMapping(result[index], EXTERNAL_MAPPINGS.lastFM[filter]),
				);
			});
		} catch (e) {
	...
	async getArtistTopSongs(artist: Artist, limit = 10): Promise<LastFM.Track[]> {
		const params: any = {
		...
		const topSongs = await axios.get("http://ws.audioscrobbler.com/2.0/", {
			params,
		});

		return topSongs.data.toptracks.track;
	}

	async getArtistAlbums(artist: Artist, limit = 10): Promise<LastFM.Album[]> {
		const params: any = {
		...
		const albums = await axios.get("http://ws.audioscrobbler.com/2.0/", {
			params,
		});
	...
	removeCachedDuplicateSongs(cachedArray: Song[], array2: any[]) {
		...
		const cachedKeys = new Set(
			cachedArray.map(
				(t) => `${t.title.toLowerCase()}:${t.artistName.toLowerCase()}`,
			),
		);
		...
		return array2.filter((externalTrack) => {
			...
			return (
				!cachedLinks.has(externalTrack.url) ||
	...
	removeCachedDuplicateArtists(cachedArray: Artist[], array2: any[]) {
		...
		return array2.filter((externalArtist) => {
			return (
				!cachedLinks.has(externalArtist.url) ||
